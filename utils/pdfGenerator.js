...
function fillTemplate(templateHtml, formData, qrCodeUrl = null, signatureImage = null, isSigned = false) {
    ...
    function formatDateWithSeparator(dateValue, separator = '/') {
        if (!dateValue) {
            return '';
        }
        ...
        if (slashMatch) {
            return `${slashMatch[1]}${separator}${slashMatch[2]}${separator}${slashMatch[3]}`;
        }

        const dashMatch = String(dateValue).match(/^(\d{2})-(\d{2})-(\d{4})/);
        if (dashMatch) {
            return `${dashMatch[1]}${separator}${dashMatch[2]}${separator}${dashMatch[3]}`;
        }
    ...
    const placeholders = {
        ...
        'CERTIFICATE_DATE_DASHED': formatDateWithSeparator(formData.CERTIFICATE_DATE, '-') || printedOn.split(' ')[0].replace(/\//g, '-'),
        'CERTIFICATE_DATE_SLASHED': formatDateWithSeparator(formData.CERTIFICATE_DATE, '/') || printedOn.split(' ')[0],
        'CERTIFICATE_DATE': formatDateWithSeparator(formData.CERTIFICATE_DATE, '/') || printedOn.split(' ')[0],
